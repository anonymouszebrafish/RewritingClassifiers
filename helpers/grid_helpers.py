import torch, copy, os, json, sys, cox, random, string, dill, git
sys.path.append('..')
import torch as ch
import torchvision
import numpy as np
...
CONFIG_ARGS = [['dataset-name', str, 'name of dataset', REQ],
               ...
               ['concept-dir', str , 'source of concepts', '/tmp/concepts'],
               ['styles', list, 'style names', REQ],
               ['style-dir', str, 'path to stylized images', '/tmp/styles'],
               ...
               ['anno-filename', str, 'annotation path', '/tmp/manual'],
               ...
               ['cache-dir', str, 'model cache', '/tmp/cache'],
               ['cov-dir', str, 'cov cache', '/tmp/cov'],
...
def find_impacted_classes(args, concept_name):
    ...
    anno_path = os.path.join(args.anno_filename, args.dataset_name, args.concepts)
